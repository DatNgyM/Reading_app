...
void main() async {
  ...
  print('🏷️ HỆ THỐNG NHÃN PDF ĐƠN GIẢN');
  print('═' * 50);

  try {
    // Tạo hệ thống nhãn đơn giản
    final labelSystem = SimplePdfLabelSystem();
    ...
    for (var testCase in testCases) {
      ...
      // Sử dụng hệ thống nhãn đơn giản
      final result = labelSystem.findContent(book, chapters);
...
class SimplePdfLabelSystem {
  // Hệ thống nhãn chính đơn giản
  final Map<String, ChapterLabel> _labels = {
    'Mt': ChapterLabel('Ma-thi-ơ', 'eBookKinhThanhTanUoc.pdf', {
      ...
      5: PageLabel('Bài giảng trên núi', [9, 10]),
    ...
    'Lc': ChapterLabel('Lu-ca', 'eBookKinhThanhTanUoc.pdf', {
    ...
    'Rm': ChapterLabel('Rô-ma', 'eBookKinhThanhTanUoc.pdf', {
    ...
    'Ga': ChapterLabel('Giăng', 'eBookKinhThanhTanUoc.pdf', {
  ...
  // Tìm nội dung
  SimpleResult findContent(String book, String chapters) {
    ...
    return SimpleResult(
      ...
      isMatch: isMatch,
      content: isMatch
          ? _getContentDescription(book, chapter, verses)
          : 'Nội dung không khớp',
    );
  }
  ...
  bool _checkMatch(PageLabel subLabel, List<int> verses) {
    return verses.isEmpty || verses.length <= 10;
  }

  String _getContentDescription(String book, int chapter, List<int> verses) {
    if (verses.isEmpty) {
...
class ChapterLabel {
  ...
  ChapterLabel(this.title, this.pdfFile, this.subLabels);
}
